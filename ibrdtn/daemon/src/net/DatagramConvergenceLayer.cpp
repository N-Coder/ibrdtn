...
namespace dtn {
    namespace net {
        ...
        void DatagramConvergenceLayer::callback_ack(DatagramConnection &, const unsigned int &seqno,
                                                    const std::string &destination) throw(DatagramException) {
            // only on sender at once
            ibrcommon::MutexLock l(_send_lock);
        ...
        void DatagramConvergenceLayer::queue(const dtn::core::Node &node, const dtn::net::BundleTransfer &job) {
            ...
            if (uri_list.empty()) {
                IBRCOMMON_LOGGER_DEBUG_TAG(TAG, 10)
                    << "discarding job for " << node << ", no URI matches"
                    << DiscoveryService::asTag(_service->getProtocol()) << IBRCOMMON_LOGGER_ENDL;
                return;
            }
        ...
        DatagramConnection &DatagramConvergenceLayer::getConnection(const std::string &identifier,
                                                                    bool create) throw(ConnectionNotAvailableException) {
        ...
        void DatagramConvergenceLayer::connectionUp(const DatagramConnection *conn) {
            IBRCOMMON_LOGGER_DEBUG_TAG(TAG, 10)
                << "Up: " << conn->getIdentifier() << IBRCOMMON_LOGGER_ENDL;
        }
        ...
        void DatagramConvergenceLayer::componentUp() throw() {
            ...
            try {
                ...
                IBRCOMMON_LOGGER_TAG(TAG, error)
                    << "bind to " << _service->getInterface().toString() << " failed (" << e.what() << ")"
        ...
        void DatagramConvergenceLayer::onAdvertiseBeacon(const ibrcommon::vinterface &iface,
                                                         const DiscoveryBeacon &beacon) throw() {
                ...
                while (_running || (_connections.size() > 0)) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            connection.ack_received(ack.seqno);
                    ...
                    try {
                        ...
                        try {
                            // Decide in which queue to write based on the src address
                            connection.data_received(segment.flags, segment.seqno, &segment.data[0], segment.len);
                    ...
                    try {
                        ...
                        conn.queue_data_to_send(queue.job);
                    } catch (const std::bad_cast &) {};

                    try {
                        Shutdown &shutdown = dynamic_cast<Shutdown &>(*action);
